    ...
    <div className="mb-[6.4rem] flex">
      {ingredients.map((ing, index) => (
        <div key={index} className="flex">
          <div className="w-30 h-30 rounded-[0.3rem]">
          ...
          <div>
            <button type="button" onClick={() => onRemove(index)} className={css.removeCardBtn}>
              <Icon name="close" size={16} className="stroke-accent" />
            </button>
            <div className="flex flex-col">
              <span className="text-[1.4rem] font-bold text-accent leading-[143%]">{ing.name}</span>
              <span className="text-[1.4rem] font-bold text-secondary leading-[143%]">
                {ing.quantity}
              </span>
            </div>
          </div>
